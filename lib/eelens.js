		...
		//for(row = 0 ; row < this.h ; row++){
			//for(col = 0 ; col < this.w ; col++){
				...
				//for(s = 0 ; s < ns ; s++){
					...
					//xnew = d.x * Math.cos(phirad) + d.y * Math.sin(phirad);
					//ynew = d.y * Math.cos(phirad) - d.x * Math.sin(phirad);
					//r1 = Math.sqrt(xnew*xnew/this.source[0].ell + ynew*ynew*this.source[0].ell);
					//I_e = Math.exp(-7.66925);
					//v += I_e*Math.pow(10.0,(-3.33071*(Math.pow((r1/this.source[0].size_px), 0.25)-1.0)));
				//}
				//this.predictedimage[i++] = v;
			//}
		//}
